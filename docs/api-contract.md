...
- 필드 모양의 최종 기준은 백엔드 Swagger(`http://localhost:8000/docs`)다. 스키마가 확정되면 `src/types/document-result.ts` 를 맞추고, 가능하면 Swagger 에서 타입을 만든다 (`docs/architecture.md` "백엔드 연동")
...
- 로컬 서버 `http://localhost:8000`, Swagger `http://localhost:8000/docs`

## 우리가 정할 것

백엔드가 결정을 넘긴 항목이다. 정해지면 백엔드에 알리고 위 "정해진 것" 으로 옮긴다.

- **웹 토큰 저장 위치**: 액세스 · 리프레시 토큰을 어디에 둘지 (프론트 결정, `docs/architecture.md`)
- **배포 도메인**: 프론트는 Vercel 의 `https://www.ssokssok.site` 다 (2026-09-15). API 를 같은 도메인의 `/api` 로 둘지(Vercel 이 백엔드로 넘김), `api.ssokssok.site` 같은 따로 된 주소로 둘지는 백엔드 배포처가 정해지면 함께 정한다
...
8. **로그인 토큰**: `POST /auth/refresh` 로 액세스 토큰을 새로 받는다. 액세스 30분 · 리프레시 14일, 로그인 요청 본문은 `{ code, redirectUri }`. 토큰 없는 변환 요청은 401(`UNAUTHORIZED`). 로그인한 사용자의 하루 이용 횟수 제한이 있다면 그 값과 code(`RATE_LIMITED`, 429)
9. **내 정보**: `/users/me` 에 `email` 을 더한다 (없으면 `null`). 설정 화면이 계정 이메일을 보여 준다. 카카오 이메일 동의 항목은 콘솔에서 설정한다
