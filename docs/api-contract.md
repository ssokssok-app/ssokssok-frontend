...
5. **오류 code 목록**: 아래 표를 기준으로 더하거나 고칠 것만 알려 준다. 화면 문구는 프론트가 code 별로 정하므로, 새 code 를 만들면 꼭 알려 준다 (모르는 code 는 "잠시 뒤에 다시 시도해주세요" 로 보인다)
   ...
   | `RATE_LIMITED`          | 429  | 오늘 이용 횟수를 다 씀. 홈으로 (13번)            |
...
13. **하루 이용 횟수** (2026-09-16 백엔드 답변, 아직 배포 전): 로그인 계정당 하루 3회. `POST /documents/convert` 를 부르는 즉시 확인해, 다 썼으면 작업을 만들기 전에 429 `RATE_LIMITED` 를 준다. 프론트는 홈에 오늘 남은 횟수를 보여 준다 (`docs/product.md` "문서 입력 · 로그인")

- 백엔드 코드에서 확인한 것 (2026-09-16, `feature/auth` 브랜치): 한국 시간 자정에 다시 채운다. 요청할 때 1회를 쓰므로 변환 실패 · 취소도 1회를 쓰고, 파일 형식 · 크기 오류(`UNSUPPORTED_FORMAT` · `FILE_TOO_LARGE`)도 횟수를 쓴 뒤에 검사한다. 사용 기록은 서버 메모리에만 있어 재배포 · 재시작하면 초기화되고, 서버가 여러 대면 따로 센다
- **백엔드에 요청할 것:** 남은 횟수 조회 API. 서버에 계산 함수(`get_remaining_conversions`)는 있지만 부를 경로가 없다. 프론트는 `GET /api/users/me/usage` (토큰 필요) → `{ "remaining": 2, "limit": 3 }` 에 맞춰 두었다 (`src/api/users.ts`). 없거나 실패하면 말풍선만 숨기고, 지금은 404 라 보이지 않는다
- **백엔드와 정할 것:** 실패한 변환도 횟수를 쓸지. 제품 결정(2026-09-15)은 "결과를 받았을 때만 줄어든다" 인데 백엔드는 재시도 남용을 막으려고 요청할 때 줄인다. 적어도 파일 형식 · 크기 오류는 횟수를 쓰기 전에 검사해 달라고 제안한다 (`docs/product.md` "확인 필요")
- 프론트는 변환을 요청할 때 저장해 둔 남은 횟수를 지워, 홈에 돌아오면 새로 받는다. 어느 쪽으로 정해져도 프론트는 바꿀 곳이 없다
