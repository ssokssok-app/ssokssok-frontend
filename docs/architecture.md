...
| `src/lib/`        | 앱 전역 인스턴스 · 유틸 (`query-client.ts`, `utils.ts`, 기능 스위치 `features.ts`, 기기 번호 `device-id.ts`)                                                                                                                                                                                                                                                                                                                                                                  |
...
- API 계약은 `docs/api-contract.md` 에 있다. Swagger 는 2026-09-17 부터 프로덕션에서 닫혀 있어 백엔드 레포에서 본다
...
- **소셜 로그인 흐름 (로그인이 꺼져 있어 지금은 쓰지 않는다. 다시 켤 때 그대로 쓴다):**
